...
public final class PotionHudRenderer {
    ...
    private static int[] renderEntries(GuiGraphics g, PotionHudConfig cfg,
        ...
        for (int i = 0; i < displayCount; i++) {
            ...
            if (e.holder() != null) {
                final int fx = iconX, fy = y, fSize = iconSize;
                e.holder().unwrapKey().ifPresent(key -> {
                    ResourceLocation id = key.location();
                    ResourceLocation sprite = ResourceLocation.fromNamespaceAndPath(
                        id.getNamespace(), "mob_effect/" + id.getPath());
                    g.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, fx, fy, fSize, fSize, 1.0f);
                });
            }
        ...
        if (hasOverflow) {
            ...
            for (int i = 0; i < Math.min(overflow.size(), 4); i++) {
                ...
                if (e.holder() != null) {
                    final int mx = iconX + col * (miniSize + miniGap);
                    final int my = y     + row * (miniSize + miniGap);
                    e.holder().unwrapKey().ifPresent(key -> {
                        ResourceLocation id = key.location();
                        ResourceLocation sprite = ResourceLocation.fromNamespaceAndPath(
                            id.getNamespace(), "mob_effect/" + id.getPath());
                        g.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, mx, my, miniSize, miniSize, 0.85f);
                    });
                }
