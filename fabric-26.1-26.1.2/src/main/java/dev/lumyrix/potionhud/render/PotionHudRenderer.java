...
public final class PotionHudRenderer {
    ...
    public static void render(GuiGraphicsExtractor graphics, DeltaTracker delta) {
        ...
        if (cfg.isPreviewMode()) {
            ...
            graphics.text(mc.font, warn, sw / 2 - tw / 2 + 1, 5, 0xFFAA0000, false);
            graphics.text(mc.font, warn, sw / 2 - tw / 2,     5, 0xFFFF3333, false);
        }
    ...
    private static List<EffectEntry> buildFakeEntries() {
        return List.of(
            new EffectEntry("Speed",        " II",  "--:--", false, 1f, MobEffects.SPEED),
            new EffectEntry("Regeneration", " I",   "--:--", false, 1f, MobEffects.REGENERATION),
            new EffectEntry("Strength",     " III", "--:--", false, 1f, MobEffects.STRENGTH),
            new EffectEntry("Jump Boost",   " II",  "--:--", false, 1f, MobEffects.JUMP_BOOST),
            new EffectEntry("Resistance",   " I",   "--:--", false, 1f, MobEffects.RESISTANCE)
        );
    }

    private static int[] renderEntries(GuiGraphicsExtractor g, PotionHudConfig cfg,
        ...
        for (int i = 0; i < displayCount; i++) {
            ...
            if (e.holder() != null) {
                Identifier spriteId = Gui.getMobEffectSprite(e.holder());
                g.blitSprite(RenderPipelines.GUI_TEXTURED, spriteId, iconX, y, iconSize, iconSize, 1.0f);
            }
        ...
        if (hasOverflow) {
            ...
            for (int i = 0; i < Math.min(overflow.size(), 4); i++) {
                ...
                if (e.holder() != null) {
                    ...
                    Identifier spriteId = Gui.getMobEffectSprite(e.holder());
                    g.blitSprite(RenderPipelines.GUI_TEXTURED, spriteId, mx, my, miniSize, miniSize, 0.85f);
                }
    ...
    private static void drawScrollingText(GuiGraphicsExtractor g, Minecraft mc, String text,
    ...
    private static void drawScaledText(GuiGraphicsExtractor g, Minecraft mc,
                                        String text, int x, int y, int color, float scale) {
        if (scale == 1.0f) { g.text(mc.font, text, x, y, color, false); return; }
        g.pose().pushMatrix();
    ...
    public static void drawRoundedRectAA(GuiGraphicsExtractor g, int x, int y, int w, int h, int r, int color) {
        if (w <= 0 || h <= 0) return;
