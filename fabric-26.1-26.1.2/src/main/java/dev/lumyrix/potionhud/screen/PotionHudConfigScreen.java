...
public class PotionHudConfigScreen extends Screen {
    ...
    private void initTabbed() {
        ...
        addRenderableWidget(Button.builder(
            ...
            .pos(this.width - 82, this.height - 22).size(78, 18).build())
    ...
    private void initSaveScreen() {
        int cx = this.width / 2 - 100, cy = this.height / 2 - 40;
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        if (showColorPicker) {
            int leftCx = this.width / 4 - 100;
            int cy = this.height / 2 - 40;
            g.centeredText(this.font, Component.translatable("potionhud.screen.color_picker"),
                this.width / 4, cy - 20, 0xFFFFFFFF);
            g.centeredText(this.font, Component.literal("#RRGGBB"), this.width / 4, cy - 10, 0xFFAAAAAA);
            int previewX = this.width * 3 / 4 - 50;
            ...
            g.centeredText(this.font, Component.literal("Preview"), this.width * 3 / 4, previewY + previewSize + 5, 0xFFAAAAAA);
            return;
        }
        if (showSaveScreen) {
            int cy = this.height / 2 - 40;
            g.centeredText(this.font, Component.translatable("potionhud.screen.save_preset"), this.width/2, cy, 0xFFFFFFFF);
            g.centeredText(this.font, Component.translatable("potionhud.hint.save_preset"), this.width/2, cy+10, 0xFFAAAAAA);
            g.centeredText(this.font, Component.translatable("potionhud.hint.config_folder"), this.width/2, cy+80, 0xFF888888);
            return;
        }
        if (showLoadScreen) {
            int cy = this.height / 2 - 40;
            g.centeredText(this.font, Component.translatable("potionhud.screen.load_preset"), this.width/2, cy, 0xFFFFFFFF);
            g.centeredText(this.font, Component.translatable("potionhud.hint.load_preset"), this.width/2, cy+10, 0xFFAAAAAA);
            g.centeredText(this.font, Component.translatable("potionhud.hint.config_folder"), this.width/2, cy+80, 0xFF888888);
            return;
        }
        ...
        g.centeredText(this.font, this.title, this.width/2, top - TAB_H - 42, 0xFFFFFFFF);
        ...
        if (activeTab == 0) {
            g.centeredText(this.font,
                ...
                this.width/2, top + 58, 0xFFCCCCCC);
        }
        if (activeTab == 1 && showAdvancedHudOptions) {
            g.text(this.font, Component.translatable("potionhud.option.max_effects_label"),
                cx, top + 173, 0xFF888888, false);
        }

        g.centeredText(this.font,
            Component.translatable("potionhud.label.preset", PotionHudConfig.getInstance().getPreset().toUpperCase()),
            this.width/2, top + 240, 0xFFAAAAAA);

        if (System.currentTimeMillis() < feedbackUntil)
            g.centeredText(this.font, Component.literal(feedbackMsg), this.width/2, top + 255, 0xFF88FF88);
