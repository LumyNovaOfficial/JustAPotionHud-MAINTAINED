...
public class HudPositionScreen extends Screen {
    ...
    private int cachedHudW = 80;
    private int cachedHudH = 36;
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        g.centeredText(
            this.font,
            Component.translatable("potionhud.position.hint"),
            this.width / 2,
            this.height - 40,
            0xFFAAAAAA
        );
    ...
    public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY) {
        if (event.button() == 0 && isDragging) { move(event.x(), event.y()); return true; }
    ...
    private void move(double mx, double my) {
        ...
        if (showGuideLines) {
            ...
            if (Math.abs(cx - this.width / 2.0f) <= SNAP) {
                nx = this.width / 2.0f - cachedHudW / 2.0f;
            }

            if (Math.abs(cy - this.height / 2.0f) <= SNAP) {
                ny = this.height / 2.0f - cachedHudH / 2.0f;
            }
        }
